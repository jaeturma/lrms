...
export default function SchoolSme({ sme, movements, catalog, categories, conditions, statuses }: Props) {
    ...
    const submit = () => {
        const options = {
            ...
            onSuccess: () => {
                setDialogOpen(false);
                toast.success(editingId === null ? 'SME item registered.' : 'SME item updated.');
            },
        };
    ...
    const deleteSme = (item: SmeItem) => {
        ...
        router.delete(`/school/sme/${item.id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('SME item removed.'),
        });
    };
                                            ...
                                            <RowActions
                                                ...
                                                actions={[
                                                    ...
                                                    { label: 'Edit', icon: Pencil, onSelect: () => openEditDialog(item) },
                                                    {
                                                        ...
                                                        icon: History,
                                                        onSelect: () =>
                                                            setHistorySmeId(historySmeId === item.id ? null : item.id),
                                                    },
                                                    {
                                                        label: 'Delete',
                                                        icon: Trash2,
                                                        variant: 'destructive',
                                                        onSelect: () => deleteSme(item),
                                                    },
                                                ]}
                                            />
                                        </td>
                        ...
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>
                                {visibleMovements.length === 0 && (
                                    <EmptyTableRow colSpan={6} message="No SME movements recorded yet." />
                                )}
