...
type EquipmentItem = {
    id: number;
    ict_equipment_catalog_item_id: number | null;
    item_code: string;
    item_name: string;
    category: string;
    brand: string | null;
...
export default function SchoolIctEquipment({ equipment, movements, catalog, categories, conditions, statuses }: Props) {
    ...
    const selectedCatalogItem = useMemo(
        () => catalog.find((item) => String(item.id) === String(data.ict_equipment_catalog_item_id)) ?? null,
        [catalog, data.ict_equipment_catalog_item_id],
    );
    ...
    const openEditDialog = (item: EquipmentItem) => {
        ...
        clearErrors();
        setData({
            ict_equipment_catalog_item_id: item.ict_equipment_catalog_item_id ? String(item.ict_equipment_catalog_item_id) : '',
    ...
    const chooseCatalogItem = (catalogItemId: string) => {
        ...
        setData({
            ...data,
            ict_equipment_catalog_item_id: String(catalogItem.id),
    ...
    const submit = () => {
        ...
        if (editingId === null) {
            post('/school/ict-equipment', options);
        } else {
            put(`/school/ict-equipment/${editingId}`, options);
        }
    };
    ...
    const deleteEquipment = (item: EquipmentItem) => {
        ...
        router.delete(`/school/ict-equipment/${item.id}`, { preserveScroll: true });
    };
            ...
            <div className="space-y-6 p-4 md:p-6">
                <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <h2 className="text-lg font-semibold text-foreground">ICT Equipment</h2>
                            <p className="text-sm text-muted-foreground">
                                Register and track laptops, desktops, tablets, and other ICT equipment. Status, condition,
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-6xl">
                    ...
                    <div className="grid gap-3">
                        ...
                        <Field label="Catalog Equipment" error={errors.ict_equipment_catalog_item_id}>
                            <select
                                value={data.ict_equipment_catalog_item_id}
                        ...
                        {editingId !== null && (
                            <Field label="Movement Notes (for this change)" error={errors.movement_notes}>
                                <Input
                                    ...
                                    placeholder="e.g. Issued to ICT coordinator for the school year"
                                />
                            </Field>
                        )}
                    </div>
