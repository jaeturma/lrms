...
export default function AdminOtherEquipmentCatalog({ filters, categories, catalogItems }: Props) {
    ...
    const submit = () => {
        ...
        if (editingId === null) {
            post('/app/admin/other-equipment-catalog', options);
        } else {
            put(`/app/admin/other-equipment-catalog/${editingId}`, options);
        }
    };
    ...
    const deleteItem = (item: CatalogItem) => {
        ...
        router.delete(`/app/admin/other-equipment-catalog/${item.id}`, { preserveScroll: true });
    };
    ...
    const submitImport = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await http.post('/app/admin/other-equipment-catalog/import', formData, {
                headers: {
                    ...
                    <header className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-4">
                                ...
                                />
                                <div>
                                    <h1 className="text-2xl font-bold text-foreground">Other Equipment Catalog</h1>
                                    <p className="text-sm text-muted-foreground">
                                        Division-managed list of TVL, ALS, Library, SPED, Sports, and other equipment ({catalogItems.total.toLocaleString()} items).
                                        Schools pick from this catalog when registering the equipment in their possession.
                                    </p>
                                </div>
                            </div>
                    ...
                    <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">Upload CSV or Excel .xlsx entries for the master equipment catalog.</p>
                            </div>
                            <a href="/app/admin/other-equipment-catalog/import/template" className="text-sm font-semibold underline">
                                Download Template
                            </a>
                        </div>
                        ...
                        <form method="get" action="/app/admin/other-equipment-catalog" className="mb-4 flex flex-wrap gap-2">
                            <input
                                name="search"
