...
export default function AdminOtherEquipmentCatalog({ filters, categories, catalogItems }: Props) {
    ...
    const submit = () => {
        const options = {
            ...
            onSuccess: () => {
                setDialogOpen(false);
                toast.success(editingId === null ? 'Equipment added to catalog.' : 'Catalog equipment updated.');
            },
        };
    ...
    const deleteItem = (item: CatalogItem) => {
        ...
        router.delete(`/app/admin/other-equipment-catalog/${item.id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('Catalog equipment removed.'),
        });
    };
                            ...
                            <table className="min-w-full text-sm">
                                ...
                                <tbody>
                                    {catalogItems.data.length === 0 && (
                                        <EmptyTableRow
                                            colSpan={7}
                                            message="No catalog equipment found. Click Add Equipment to create the first entry."
                                        />
                                    )}
