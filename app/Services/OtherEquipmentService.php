...
class OtherEquipmentService
{
     ...
     */
    public function create(School $school, array $data, ?User $user = null): OtherEquipment
    {
        return DB::transaction(function () use ($school, $data, $user): OtherEquipment {
            $equipment = $school->otherEquipment()->create($data);

            if (blank($equipment->item_code)) {
                $equipment->item_code = sprintf('OTH-%s-%05d', now()->year, $equipment->id);
            }
     ...
     * @param  array<string, mixed>  $data
     */
    public function update(OtherEquipment $equipment, array $data, ?User $user = null): OtherEquipment
    {
        return DB::transaction(function () use ($equipment, $data, $user): OtherEquipment {
    ...
    public function delete(OtherEquipment $equipment, ?User $user = null): void
    {
