...
class EquipmentService
{
     ...
     */
    public function create(School $school, array $data, ?User $user = null): Equipment
    {
        return DB::transaction(function () use ($school, $data, $user): Equipment {
            $equipment = $school->equipment()->create($data);

            if (blank($equipment->item_code)) {
                $equipment->item_code = sprintf('EQP-%s-%05d', now()->year, $equipment->id);
            }
     ...
     * @param  array<string, mixed>  $data
     */
    public function update(Equipment $equipment, array $data, ?User $user = null): Equipment
    {
        return DB::transaction(function () use ($equipment, $data, $user): Equipment {
    ...
    public function delete(Equipment $equipment, ?User $user = null): void
    {
