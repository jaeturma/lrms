...
class StoreIctEquipmentRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            'ict_equipment_catalog_item_id' => [
                'nullable',
                'integer',
                Rule::exists('ict_equipment_catalog_items', 'id')
                    ->where(function ($query): void {
                        $query
                            ->where('is_active', true)
                            ->orWhere('id', $this->route('ictEquipment')?->ict_equipment_catalog_item_id ?? 0);
                    }),
            ],
            'item_code' => [
                'nullable',
                'string',
                'max:50',
                Rule::unique('ict_equipment', 'item_code')->ignore($this->route('ictEquipment')?->id),
            ],
            ...
            'condition' => ['required', 'string', Rule::in(IctEquipment::CONDITIONS)],
            'status' => ['required', 'string', Rule::in(IctEquipment::STATUSES)],
    ...
    {
        $equipment = $this->route('ictEquipment');
        $catalogItem = $this->input('ict_equipment_catalog_item_id')
            ? IctEquipmentCatalogItem::query()
                ->whereKey($this->input('ict_equipment_catalog_item_id'))
                ->where(function ($query) use ($equipment): void {
                    $query
                        ->where('is_active', true)
                        ->orWhere('id', $equipment?->ict_equipment_catalog_item_id ?? 0);
                })
                ->first()
            : null;
